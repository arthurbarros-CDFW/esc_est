...
// [[Rcpp::export]]
List cpp_pro_capsur(int i, int j, NumericVector beta, NumericMatrix ch, NumericMatrix cap_X, NumericMatrix surv_X, NumericVector ints) {
  int p = beta.size();
  ...
  double s_hat = zs / (1 + zs);
   ...
   // Apply interval to survival probability
  if(ints.size() > j-1) {
    s_hat = pow(s_hat, ints[j-1]);
  }
...
// [[Rcpp::export]]
double cpp_CJSloglik(NumericVector beta, NumericMatrix ch, NumericMatrix cap_X, NumericMatrix surv_X, NumericVector ints) {
  double xlnlik = 0;
  int nan = ch.nrow();
  int ns = ch.ncol();
  ...
  for (int i = 0; i < nan; ++i) {
    ...
    if (init1 <= ns) {
      for (int j = init1; j <= ns; ++j) {
        vp_ij[j - 1] = cpp_pro_capsur(i, j, beta, ch, cap_X, surv_X, ints)["p.hat"];
      }
    }

    if (init2 < ns) {
      for (int j = init2; j < ns; ++j) {
        vs_ij[j-1] = cpp_pro_capsur(i, j, beta, ch, cap_X, surv_X, ints)["s.hat"];
      }
    }
...
// [[Rcpp::export]]
double cpp_CJSwrapper(NumericVector beta, NumericMatrix ch, NumericMatrix cap_X, NumericMatrix surv_X, NumericVector ints) {
  double lnlik = -1 * cpp_CJSloglik(beta, ch, cap_X, surv_X, ints);
  return lnlik;
}

// [[Rcpp::export]]
List cpp_optim(NumericVector beta, NumericMatrix ch, NumericMatrix cap_X, NumericMatrix surv_X, NumericVector ints) {
